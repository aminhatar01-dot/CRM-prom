...
export const agentConfigSchema = z.object({
  ...
  formality: z
    .enum(["very_informal", "close", "professional", "very_formal"])
    .default("professional"),
  response_length: z
    .enum(["very_short", "normal", "detailed"])
    .default("normal"),
  emoji_usage: z.enum(["never", "low", "normal", "frequent"]).default("low"),
  commercial_pace: z
    .enum(["calm", "consultative", "commercial", "aggressive"])
  ...
  can_answer_prices: z.boolean().default(false),
  can_create_quotes: z.boolean().default(false),
  can_send_quotes: z.boolean().default(false),
  quote_requires_human_approval: z.boolean().default(true),
  can_auto_send_simple_prices: z.boolean().default(false),
  can_auto_send_full_quotes: z.boolean().default(false),
  quote_auto_send_max_amount: z
    .number()
    .finite()
    .positive()
    .nullable()
    .default(null),
  missing_price_behavior: z.enum(["human", "ask"]).default("human"),
  missing_stock_behavior: z.enum(["confirm", "human"]).default("confirm"),
  quote_knowledge_categories: textList,
  default_currency: z
    .string()
    .regex(/^[A-Z]{3}$/)
    .default("ARS"),
  default_commercial_terms: z.string().trim().max(2000).default(""),
});
...
export const agentPlaybooksSchema = z
  .array(agentPlaybookSchema)
  .max(12)
  .default([]);
...
export function buildAgentRuntime(
  ...
  const prompt = [
    ...
    config.business_description
      ? `Descripcion del negocio: ${config.business_description}`
      : "",
    ...
    config.can_answer_prices
      ? `Puedes responder precios solo cuando exista una coincidencia clara en estas categorias: ${config.quote_knowledge_categories.join(", ") || config.knowledge_categories.join(", ") || "catalogo habilitado"}.`
      : "No tienes permiso para confirmar precios; deriva la consulta al asistente correspondiente o a una persona.",
    config.can_create_quotes
      ? `Puedes reunir items y cantidades para cotizar en ${config.default_currency}. ${config.default_commercial_terms ? `Condiciones por defecto: ${config.default_commercial_terms}` : "No inventes condiciones comerciales."}`
      : "No tienes permiso para crear cotizaciones.",
    config.quote_requires_human_approval
      ? "Toda cotizacion completa requiere aprobacion humana antes de enviarse."
      : "Una cotizacion solo puede autoenviarse si todos los controles de seguridad y monto lo permiten.",
    config.missing_price_behavior === "human"
      ? "Si falta un precio, deriva a una persona."
      : "Si falta un precio, pide el dato o explica que debe confirmarse.",
    config.missing_stock_behavior === "human"
      ? "Si falta stock, deriva a una persona."
      : "Si falta stock, pide confirmacion y no afirmes disponibilidad.",
  ...
  const rules = [
    ...
    ...config.create_task_when.map(
      (value) => `Sugerir crear tarea cuando: ${value}`,
    ),
    ...config.create_opportunity_when.map(
      (value) => `Sugerir oportunidad cuando: ${value}`,
    ),
    ...config.create_appointment_when.map(
      (value) => `Sugerir cita cuando: ${value}`,
    ),
    ...config.pause_ai_when.map((value) => `Pausar IA cuando: ${value}`),
    ...config.auto_reply_when.map(
      (value) => `Auto respuesta permitida cuando: ${value}`,
    ),
    ...config.draft_only_when.map((value) => `Solo borrador cuando: ${value}`),
  ];

  const tone =
    config.formality === "very_formal" ||
    config.communication_style === "executive"
      ? "professional"
      : config.formality === "close" ||
          config.communication_style === "friendly"
        ? "warm"
        : "direct";
