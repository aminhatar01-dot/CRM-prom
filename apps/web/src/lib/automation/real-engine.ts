...
import { buildConversationVariableContext, mapVariable, type VariableRow } from "@/lib/ai/variable-context";
import {
  enforceAIRateLimit,
  getAIRuntimeConfig,
  summarizeAIInput,
...
export async function dispatchAutomationEvent(supabase: SupabaseClient, event: EventInput) {
  ...
  if (error) throw new Error(`Automation rules could not be loaded: ${error.message}`);
...
async function createAndExecuteRun(supabase: SupabaseClient, rule: RuleRow, event: EventInput) {
  ...
  if (error || !run) throw new Error(`Automation run could not be created: ${error?.message ?? "unknown"}`);
...
export async function executeRealAutomationRun(
  ...
  const rule = suppliedRule ?? await loadRule(supabase, run.organization_id, run.rule_id);
  ...
  try {
    if (!conditionsMatch(rule.conditions, context)) {
      await finish(supabase, run, "cancelled", { matched: false, reason: "conditions_not_met" });
    ...
    for (const action of actions ?? []) {
      actionResults.push(await executeAction(supabase, rule, run, context, action));
    }
    ...
    await supabase.from("automation_rules").update({ last_run_at: new Date().toISOString() })
      .eq("id", rule.id).eq("organization_id", run.organization_id);
...
async function executeAction(
  ...
  try {
    ...
    if (action.action_type === "create_task") {
      ...
      const { error } = await supabase.from("leads").update({ status })
        .eq("id", run.lead_id).eq("organization_id", run.organization_id);
      if (error) throw error;
    ...
    } else if (action.action_type === "generate_ai_draft" || action.action_type === "send_message") {
      output = await generateDraft(supabase, rule, run, context, action.config);
    } else if (action.action_type === "pause_ai") {
      if (!run.conversation_id) throw new Error("Conversation context is required.");
    ...
    await supabase.from("automation_execution_logs").insert({
      ...logBase,
      status: output.blocked ? "blocked" : output.skipped ? "skipped" : "completed",
...
async function generateDraft(
  ...
  const aiContext = await buildConversationAIContext({
    ...
    userInput: typeof config.instruction === "string" ? config.instruction : undefined
  });
  ...
  const { data: draft, error } = await supabase.from("automation_drafts").insert({
    organization_id: run.organization_id,
    rule_id: rule.id,
    run_id: run.id,
    conversation_id: run.conversation_id,
...
export async function sendDraft(
  ...
  try {
    ...
    await supabase.from("messages").update({
      ...
      metadata: { automation_draft_id: draftId, auto_send: !approvedBy, whatsapp_response: response }
...
async function extractVariable(
  supabase: SupabaseClient,
  run: { organization_id: string; conversation_id: string | null; lead_id: string | null },
  config: Record<string, unknown>,
) {
  if (!run.conversation_id) return { skipped: true, reason: "conversation_missing" };
  let query = supabase.from("variables")
    .select("id, organization_id, name, key, description, type, extraction_prompt, active, required, options")
    .eq("organization_id", run.organization_id).eq("active", true).is("archived_at", null);
  ...
  await enforceAIRateLimit(supabase, run.organization_id);
  const extraction = await new VariableExtractor(getAIRuntimeConfig()).extract(variables, context);
  let count = 0;
...
async function updateVariable(
  ...
  if (run.conversation_id) await supabase.from("conversation_variables").upsert({
    ...payload, conversation_id: run.conversation_id
  }, { onConflict: "conversation_id,variable_id" });
  if (run.lead_id) await supabase.from("lead_variables").upsert({
    ...payload, lead_id: run.lead_id
  }, { onConflict: "lead_id,variable_id" });
  return { variable_id: variableId, updated: true };
}

async function loadContext(supabase: SupabaseClient, event: EventInput): Promise<LoadedContext> {
  ...
  if (leadId) {
    const { data: lead } = await supabase.from("leads").select("status, owner_id")
      .eq("id", leadId).eq("organization_id", event.organizationId).maybeSingle<{ status: string; owner_id: string | null }>();
...
async function loadRule(supabase: SupabaseClient, organizationId: string, ruleId: string) {
  const { data } = await supabase.from("automation_rules")
    .select("id, organization_id, name, trigger_type, status, conditions, trigger_config, auto_send, auto_reply_limit, auto_reply_window_minutes")
    .eq("id", ruleId).eq("organization_id", organizationId).single<RuleRow>();
